...
class BoardConfigPatch(BaseModel):
    ...
    is_eligible: StrictBool | None = None
...
def _board_payload(db, r):
    return {
        "id": r.id,
        "external_board_id": r.external_board_id,
        "name": r.name,
        ...
        "is_ads_only": r.is_ads_only,
        "image_cover_url": r.image_cover_url,
        "board_pins_modified_at": r.board_pins_modified_at,
        "provider_created_at": r.provider_created_at,
        "is_active": r.is_active,
        "is_eligible": r.is_eligible,
        "routing_label": r.routing_label,
        "last_seen_at": r.last_seen_at,
        "last_synced_at": r.last_synced_at,
        "sections": [
            {
                "id": s.id,
                "external_section_id": s.external_section_id,
                "name": s.name,
                "is_active": s.is_active,
                "last_seen_at": s.last_seen_at,
                "last_synced_at": s.last_synced_at,
            }
            for s in db.scalars(select(PinterestBoardSection).where(PinterestBoardSection.board_id == r.id)).all()
        ],
    }


def _last_synced(db, connection_id):
    from sqlalchemy import func

    return db.scalar(select(func.max(PinterestBoard.last_synced_at)).where(PinterestBoard.connection_id == connection_id))
...
async def pinterest_boards_sync(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "last_synced_at": connection.boards_last_synced_at,
...
def pinterest_board_config(board_id: str, payload: BoardConfigPatch, db: Session = Depends(get_db)):
    ...
    if "is_eligible" in values and values["is_eligible"] is not None:
        row.is_eligible = values["is_eligible"]
    if "routing_label" in values:
        row.routing_label = values["routing_label"]
    db.commit()
    return {"id": row.id, "is_eligible": row.is_eligible, "routing_label": row.routing_label}
...
def _result_redirect(result: str) -> str:
    ...
    query.update({"provider": "pinterest", "result": result})
...
def channel_capabilities():
    ...
    return channel_capability_payload(publishing_enabled=get_settings().publishing_enabled)
...
async def pinterest_callback(
    ...
    claimed = db.execute(
        update(PinterestOAuthState)
        .where(
            PinterestOAuthState.state_hash == digest,
            PinterestOAuthState.consumed_at.is_(None),
            PinterestOAuthState.expires_at >= now,
        )
        .values(consumed_at=now)
    )
    if claimed.rowcount != 1:
        db.rollback()
        return RedirectResponse(_result_redirect("invalid_state"))
    db.commit()
    if error:
    ...
    try:
        ...
        if not isinstance(account, dict):
            raise RuntimeError("invalid account payload")
        external_id = account.get("id")
        if isinstance(external_id, bool) or not isinstance(external_id, (str, int)) or not str(external_id).strip():
        ...
        access_cipher = encrypt_token(tokens["access_token"])
        refresh_cipher = encrypt_token(tokens["refresh_token"])
        connection = PinterestConnection(
            external_user_id=str(external_id).strip(),
            ...
            refresh_token_expires_at=expiry("refresh_token_expires_in"),
            last_verified_at=now,
            token_type=tokens.get("token_type"),
        )
        db.query(PinterestConnection).filter(PinterestConnection.status == "CONNECTED").update(
