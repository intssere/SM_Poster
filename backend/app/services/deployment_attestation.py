...
def read_build_provenance(path: Path = DEFAULT_MANIFEST_PATH) -> BuildProvenance:
    ...
    release_tree_sha = payload.get("release_tree_sha")
    topology = payload.get("topology")
    overlay = payload.get("release_overlay")
    if not (
        isinstance(commit_sha, str) and _SHA_RE.fullmatch(commit_sha)
        and isinstance(tree_sha, str) and _SHA_RE.fullmatch(tree_sha)
        and isinstance(release_commit_sha, str) and _SHA_RE.fullmatch(release_commit_sha)
        and isinstance(release_tree_sha, str) and _SHA_RE.fullmatch(release_tree_sha)
        and topology in _TOPOLOGIES
        ...
        return BuildProvenance(present=True, valid=False, error="BUILD_PROVENANCE_INVALID")
    if topology == "canonical_with_worktree_overlay" and (
