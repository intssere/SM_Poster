...
def test_build_preview_real_db_is_server_derived_and_sanitized():
    ...
    db.add(PublicationAttempt(publication_id=publication.id, attempt_number=1, status="UNKNOWN", request_fingerprint="internal-secret", safe_response_metadata={"validated_pin_id": "1234567890", "access_token": "secret", "request_id": "token-like-value"}))
    db.commit()
    ...
    assert attempt["safe_response_metadata"] == {"validated_pin_id": "1234567890"}
