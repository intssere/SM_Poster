...
@pytest.mark.asyncio
async def test_paginated_sync_upserts_and_preserves_local_config(db, monkeypatch):
    ...
    await sync_boards(db, conn, fake)
...
@pytest.mark.asyncio
async def test_successful_full_sync_inactivates_missing_boards_and_sections(db, monkeypatch):
    monkeypatch.setattr("app.services.pinterest_boards.decrypt_token", lambda _: "token")
...
@pytest.mark.asyncio
async def test_partial_provider_failure_does_not_inactivate_existing(db, monkeypatch):
    monkeypatch.setattr("app.services.pinterest_boards.decrypt_token", lambda _: "token")
    ...
    with pytest.raises(RuntimeError): await sync_boards(db, conn, Broken({}))
